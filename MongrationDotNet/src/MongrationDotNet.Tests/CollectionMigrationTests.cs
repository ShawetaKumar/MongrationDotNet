...
namespace MongrationDotNet.Tests
{
    public class CollectionMigrationTests : TestBase
    {
        ...
        public async Task Migration_ShouldSaveMigrationDetails_WhenMigrationIsApplied()
        {
            ...
            var result = await migrationCollection.Find(x => x.Type == Constants.CollectionMigrationType).FirstOrDefaultAsync();
        ...
        public async Task Migration_ShouldSkipMigration_WhenMigrationVersionIsAlreadyApplied()
        {
            ...
            {
                Version = Version,
                Type = Constants.CollectionMigrationType,
            ...
            await MigrationRunner.Migrate();
            var result = await migrationCollection.Find(x=> x.Version == Version && x.Type == Constants.CollectionMigrationType).ToListAsync();
        ...
        {
            await MigrationRunner.Migrate();
            var result = await migrationCollection.Find(x => x.Version == Version && x.Type == Constants.CollectionMigrationType).FirstOrDefaultAsync();
